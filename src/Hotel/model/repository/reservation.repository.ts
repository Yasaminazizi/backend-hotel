...
export class ReservationRepository {
  ...
  //for method search for all rooms
  async findOverlappingReservationsForAllRooms(roomIds: string[], checkIn: Date, checkOut: Date): Promise<Reservation[]> {
    return await this.reservationRepository
      .createQueryBuilder('reservation')
      .where('reservation.roomId IN (:...roomIds)', { roomIds })
      .andWhere('reservation.status = :status', { status: 'active' })
      .andWhere('reservation.checkOutDate > :checkIn', { checkIn })
      .andWhere('reservation.checkInDate < :checkOut', { checkOut })
      .getMany();
